...
const getModalidadPaginados = async (NoPagina = 1) => {
  try {
    let url = `licenciamiento/modalidad/${NoPagina}/paginado`
    loading = true
    let response = await axios.get(url)
    loading = false
    modalidadAPI.value = response.data.items
    totalPaginas = response.data.totalPaginas
    paginaActual = response.data.paginaActual
    if (response.status == 200) {
      console.log('modalidad cargados')
    }
  } catch (error) {
...
//FUNCION QUE BUSCAR LA MODLIDAD A EDITAR Y PINTAR SUS DATOS EN EL FORMULARIO PARA SU POSTERIOR EDICION
const editarModalidad = async (item, index) => {
  expandir.value = false
  indice.value = index
  let url = `licenciamiento/modalidad/${item.id}/`
  axios.get(url)
      .then((response) => {
...
const listaFiltrada = () => modalidadAPI.value.filter((item) =>
...
const getPaginaAnterior = () => {
  ...
  getModalidadPaginados(paginaActual)
}
    ...
    <section class="section dashboard">
        ...
        <div class="col-lg-12">
          <div class="card glassmorphism">
            <ul class="sidebar-nav p-2" id="cardFormulario">
              <a class="nav-link collapsed" style="background: None" data-bs-target="#formularioModalidad"
                 data-bs-toggle="collapse" href="#"
                 aria-expanded="false"> <i class="bi-plus-circle"></i>
                <strong>{{ indice == -1 ? 'Nueva modalidad' : 'Actualizar modalidad' }}</strong><i
                    class="bi bi-chevron-down ms-auto"></i>  </a>

            </ul>
            <div class="card-body nav-content" :class="expandir ? 'collapse':''" id="formularioModalidad"
      ...
      <div class="row">
        <div class="col-lg-12">
          <div class="card glassmorphism">
            <div class="card-body">
              <h5 class="card-title"><i class="bi bi-layout-text-window-reverse"></i><strong> Listado de Modalidades</strong></h5>
              <div class="row">
                <div class="col-sm-2">
              ...
              <div class="table-responsive">
                <nav aria-label="Page navigation example">
                  <ul class="pagination justify-content-end">
                    ...
                    <li v-for="pagina in totalPaginas" @click="getModalidadPaginados(pagina)" class="page-item"
                    ...
                    <li class="page-item" @click="getModalidadPaginados(paginaActual)"><a class="page-link" title="Refrescar" href="#" aria-label="Next">
                ...
                <table class="table table-striped">
                  ...
                  <tr v-for="(item, index) in listaFiltrada()" :key="item.id">
                    <td>
                        <span class="sombra badge bg-primary" @click="editarModalidad(item, index)" title="Modificar"><i
                            class="bi bi-pencil"></i></span>
                      <span class="sombra badge bg-danger m-lg-1" @click="DELETE('licenciamiento/modalidad/'+item.id)"
