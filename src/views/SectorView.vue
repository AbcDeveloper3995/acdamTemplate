...
//PETICION GET PARA OBTENER LOS SECTOR YA PAGINADOS
const getSectorPaginados = async (NoPagina = 1) => {
  try {
    let url = `licenciamiento/sector/${NoPagina}/paginado`
    loading = true
    let response = await axios.get(url)
    loading = false
    sectorAPI.value = response.data.items
...
//FUNCION QUE BUSCAR EL SECTOR A EDITAR Y PINTAR SUS DATOS EN EL FORMULARIO PARA SU POSTERIOR EDICION
const editarSector = async (item, index) => {
  expandir.value = false
  indice.value = index
  let url = `licenciamiento/sector/${item.id}/`
  axios.get(url)
      .then((response) => {
...
const listaFiltrada = () => sectorAPI.value.filter((item) =>
...
const getPaginaAnterior = () => {
  ...
  getSectorPaginados(paginaActual)
}
    ...
    <section class="section dashboard">
        ...
        <div class="col-lg-12">
          <div class="card glassmorphism">
            <ul class="sidebar-nav p-2" id="cardFormulario">
              <a class="nav-link collapsed" style="background: None" data-bs-target="#formularioSector"
                 data-bs-toggle="collapse" href="#"
                 aria-expanded="false"> <i class="bi-plus-circle"></i>
                <strong>{{ indice == -1 ? 'Nuevo sector' : 'Actualizar sector' }}</strong><i
                    class="bi bi-chevron-down ms-auto"></i>  </a>

            </ul>
            <div class="card-body nav-content" :class="expandir ? 'collapse':''" id="formularioSector"
              ...
              <form class="row g-3">
                ...
                <div class="text-center">
                  <button @click="POST_PUT('licenciamiento/sector/', dataPost, indice)"  class="miBtn btn btn-outline-light" type="button">
      ...
      <div class="row">
        <div class="col-lg-12">
          <div class="card glassmorphism">
            <div class="card-body">
              <h5 class="card-title"><i class="bi bi-layout-text-window-reverse"></i><strong> Listado de Sectores</strong></h5>
              <div class="row">
                <div class="col-sm-2">
              ...
              <div class="table-responsive">
                <nav aria-label="Page navigation example">
                  <ul class="pagination justify-content-end">
                    ...
                    <li v-for="pagina in totalPaginas" @click="getSectorPaginados(pagina)" class="page-item"
                    ...
                    <li class="page-item" @click="getSectorPaginados(paginaActual)"><a class="page-link" title="Refrescar" href="#" aria-label="Next">
                ...
                <table class="table table-striped">
                  ...
                  <tr v-for="(item, index) in listaFiltrada()" :key="item.id">
                    <td>
                        <span class="sombra badge bg-primary" @click="editarSector(item, index)" title="Modificar"><i
                            class="bi bi-pencil"></i></span>
                      <span class="sombra badge bg-danger m-lg-1" @click="DELETE('licenciamiento/sector/'+item.id)"
